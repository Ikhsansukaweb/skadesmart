...
import { UtensilsCrossed, PackagePlus, PackageCheck, CircleDot } from "lucide-react";
...
// Overview Dashboard Brital - layout sidebar + panel kanan (bagian 7 prompt
// redesign). Data yang dipakai semuanya dari endpoint yang sudah ada
// (/orders/stats, /orders/incoming, /account) - tidak ada endpoint baru.
    ...
    <DashboardShell
      ...
      rightPanel={
        <>
          ...
          <UnitCard unitName="Ayam Geprek Brital" statusLabel={shopOpen ? "Toko sedang buka" : "Toko sedang tutup"} isOpen={!!shopOpen} />
          <ExportCard label="Riwayat penjualan Brital" rows={history.map((o) => ({
            id: o.id, pembeli: o.buyer_name, kelas: o.buyer_class, total: o.total_price, status: o.status, tanggal: o.created_at,
          }))} />
        </>
      }
    >
      <div className="flex items-center gap-2 mb-1">
        <UtensilsCrossed size={18} className="text-electric" aria-hidden="true" />
        <span className="font-sub text-sm text-steel">Ayam Geprek Brital</span>
        {shopOpen !== null && (
          <button
            onClick={toggleShop}
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        <ListCard title="Pesanan Masuk" seeAllHref="/dashboard/brital/orders" emptyLabel={incoming.length === 0 ? "Belum ada pesanan aktif." : undefined}>
        ...
        <ListCard title="Riwayat Terbaru" seeAllHref="/dashboard/brital/history" emptyLabel={history.length === 0 ? "Belum ada riwayat." : undefined}>
